...
def get_archlog_data(db_list,new_workbook):
    try:
        ...
        for index in range(len(db_list)):
            ...
            new_sheet = new_workbook.get_sheet(2)
            if db_list[index] == '10.114.130.9':
                msg1 = arch_info['10.114.130.9'] + data[0][0]
                new_sheet.write(index + 2, 7, msg1, style)
                logger.info("'%s'获取和写入归档相关监控指标数据成功！" % db_list[index])
            elif db_list[index] == '10.114.130.8':
                msg2 = arch_info['10.114.130.8'] + data[0][0]
                new_sheet.write(index + 2, 7, msg2, style)
                logger.info("'%s'获取和写入归档相关监控指标数据成功！" % db_list[index])
            elif db_list[index] == '10.114.130.121':
                msg3 = arch_info['10.114.130.121'] + data[0][0]
                new_sheet.write(index + 2, 7, msg3, style)
                logger.info("'%s'获取和写入归档相关监控指标数据成功！" % db_list[index])
            elif db_list[index] == '10.114.130.85_R':
                msg4 = arch_info['10.114.130.85_R'] + data[0][0]
                new_sheet.write(index + 2, 7, msg4, style)
                logger.info("'%s'获取和写入归档相关监控指标数据成功！" % db_list[index])
            elif db_list[index] == '10.114.130.85_A':
                msg5 = arch_info['10.114.130.85_A'] + data[0][0]
...
def write_sqlserver_img_to_excel(wb):
    ...
    if os.path.exists(img_path):
        ...
        driver.set_page_load_timeout(300)
        driver.set_script_timeout(300)
        driver.get(r"http://scom:5tgb-^YHN@10.99.60.49/ReportServer_SQL2012/Pages/ReportViewer.aspx?%2fDBA%2fDBA%e6%80%bb%e4%bd%93%e7%9b%91%e6%8e%a7&rs:Command=Render")
        time.sleep(5)
